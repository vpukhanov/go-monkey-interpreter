...
if (10 > 1) {
	...
	return 1;
		...
		{"2; fn(x) { return x; }(5); 9; return 7; 10", 7},
	}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong parameters. got=%+v",
			fn.Parameters)
	}
...
func TestStringLiteral(t *testing.T) {
	...
	if str.Value != "Hello World!" {
		t.Errorf("String has wrong value. expected=%q, got=%q",
			"Hello World!", str.Value)
	}
}
...
func TestArrayLiterals(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("array does not have 3 elements. got=%d", len(result.Elements))
	}
